...
def calculate_jitter(audio, sr):
    ...
    النتيجة هنا ليست بديلاً عن أدوات القياس الصوتي الطبية.
    """
...
:::writing{variant="document" id="74106" title="متطلبات المشروع"}
...
يفتح التطبيق في المتصفح، وعند الضغط على "st.audio_input" سيطلب المتصفح إذن الوصول إلى الميكروفون. Streamlit يعيد التسجيل كملف WAV، ويمكن تحديد معدل العينة إلى "16000 Hz".

ملاحظة مهمة: "st.audio_input" لا يفرض على المتصفح إيقاف التسجيل تلقائياً بعد 5 ثوانٍ؛ الكود أعلاه يضمن أن التحليل نفسه يستخدم 5 ثوانٍ فقط (يقتطع التسجيل إن كان أطول ويضيف padding إن كان أقصر).

أما نموذج Hugging Face فهو نموذج Speech Emotion Recognition؛ النموذج متعدد اللغات يصنف المشاعر مثل "anger", "fear", "happiness", "neutral" و"sadness"، وليس نموذجاً مُدرباً خصيصاً لإخراج «نسبة توتر». لذلك جعلت النسبة في الكود مؤشراً تجريبياً مبنياً على المشاعر + Pitch + Jitter، بدلاً من تقديمها كقياس طبي حقيقي.

إذا كان الهدف مشروعاً أكثر دقة، فالخطوة التالية هي تدريب/استخدام نموذج Stress Detection مخصص على بيانات صوتية موسومة بالتوتر، بدلاً من تحويل تصنيف المشاعر إلى نسبة توتر.import io
import numpy as np
import streamlit as st
import librosa
import soundfile as sf
